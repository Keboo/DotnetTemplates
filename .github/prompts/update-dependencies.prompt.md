...
Update all dependencies across this repository to their latest stable versions. This includes NuGet packages, dotnet tools, pnpm packages, GitHub Actions, and Terraform providers.
...
- Commit dependency updates in logical groups (NuGet, pnpm, GitHub Actions, etc.) for easier review.
...
Update pnpm dependencies for the React frontend in the Aspire template.
...
This will update `package.json` version ranges and regenerate `pnpm-lock.yaml`.
...
- After updating, ensure the lock file is regenerated by running `pnpm install`.
